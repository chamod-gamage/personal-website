...
export const Home = () => {
          ...
          <Col lg={9}>
            <Row style={{ textAlign: 'left', paddingTop: 0 }}>
              <intro>
                ...
                <br />
                <em>
                  I'm currently looking for Winter 2022 internships in software
  ...
  const jobs = [
    ...
    },
    {
      ...
      <ul>
        <li>Grocery Claims Portal: An API + pair of web applications built using React, Node.js, TypeORM, and PostgreSQL which facilitate the process of allowing grocery stores to make claims on imperfectly delivered alcohol (shortages, damage, expired goods, etc.)</li>
        <li>Electronic Bin Tag Pricing Pipeline: A data pipeline which facilitates the transition of stores from paper pricing tags to electronic tags. Built using Flask and harnessing ML for OCR, the application centralizes all pricing changes received via PDFs and provides them to external providers via an API</li>
        <li>LCBO|Desk: A custom issue servicing, ticketing, and handling system which allows stakeholders to file tickets regarding existing applications hosted by LCBO|next, and also serves as a Project Management tool for the lab for servicing these issues. </li>
      </ul>
